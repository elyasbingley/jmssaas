-- Reverts part of 20261002000100_fix_referral_automation.sql: the user
-- clarified that referral_lead_received should only fire when a referral
-- partner is actually linked to a job card, not the moment the partner
-- record itself is created (my earlier fix assumed the latter from the
-- trigger_key's name and the user's own test method, but that wasn't the
-- intended semantics after all). Drop the partner-insert trigger/function
-- and restore the original "thanks for referring {referred_client_name}"
-- wording, which only ever renders correctly on the job-tagging path this
-- keeps as the sole trigger.

drop trigger if exists schedule_referral_lead_received_on_partner_insert_trigger on public.referral_partners;
drop function if exists public.schedule_referral_lead_received_on_partner_insert();

update public.communication_templates
set body = 'Hi {partner_first_name}, thanks for referring {referred_client_name}! We have reached out to them and will keep you posted.'
where trigger_key = 'referral_lead_received'
  and type = 'email'
  and body = 'Hi {partner_first_name}, thanks for the referral! We''ve got it and will keep you posted.';
